...
class ConnectorService(connector_sdk_pb2_grpc.ConnectorServicer):
    ...
    def Update(self, request, context):
        ...
        state["last_known_synced_record"] = state.get(
            "last_known_synced_record", None)
        state["last_date_synced_to"] = state.get("last_date_synced_to", None)
        ...
        wave_range = fetcher.wave_range_to_sync(
            account_brand_ids, state["last_date_synced_to"])
        ...
        sync = True
        if state["last_known_synced_record"]:
            sync = False

        for metric in funnel_metrics:
            if not sync:
                if metric.id == state["last_known_synced_record"]:
                    print("Resuming sync")
                    sync = True
                    continue
